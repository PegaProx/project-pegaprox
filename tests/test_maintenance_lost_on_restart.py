...
#
# Consequence on the production cluster "Janus" (2026-09-10): after a container restart PegaProx
...
#
# The payloads below are captured verbatim from Janus (PVE 9.2.10) while pve-dmz-node21-th-D and
# pve-dmz-node22-th-D were in maintenance.
...
N21 = 'pve-dmz-node21-th-D'
N22 = 'pve-dmz-node22-th-D'


# --- captured from Janus: GET /cluster/ha/status/current (HTTP 200) -----------------------
JANUS_HA_STATUS_CURRENT = [
    {"id": "quorum", "node": "pve-dmz-node01-th-A", "quorate": 1, "status": "OK", "type": "quorum"},
    {"id": "master", "node": "pve-dmz-node11-th-I", "type": "master", "timestamp": 1789062521,
     "status": "pve-dmz-node11-th-I (active, Thu Sep 10 19:48:41 2026)"},
    {"armed-state": "armed", "id": "fencing", "node": "pve-dmz-node11-th-I", "type": "fencing",
     "status": "armed (CRM watchdog active)"},
    {"id": "lrm:pve-dmz-node01-th-A", "node": "pve-dmz-node01-th-A", "type": "lrm",
     "timestamp": 1789062525,
     "status": "pve-dmz-node01-th-A (idle, watchdog standby, Thu Sep 10 19:48:45 2026)"},
    {"id": "lrm:pve-dmz-node02-th-A", "node": "pve-dmz-node02-th-A", "type": "lrm",
     "timestamp": 1789062527,
     "status": "pve-dmz-node02-th-A (active, watchdog active, Thu Sep 10 19:48:47 2026)"},
    {"id": "lrm:pve-dmz-node11-th-I", "node": "pve-dmz-node11-th-I", "type": "lrm",
     "timestamp": 1789062525,
     "status": "pve-dmz-node11-th-I (idle, watchdog standby, Thu Sep 10 19:48:45 2026)"},
    {"id": "lrm:pve-dmz-node12-th-I", "node": "pve-dmz-node12-th-I", "type": "lrm",
     "timestamp": 1789062525,
     "status": "pve-dmz-node12-th-I (idle, watchdog standby, Thu Sep 10 19:48:45 2026)"},
    ...
    {"auto-rebalance": 0, "comment": "keep alive at all costs", "crm_state": "started",
     "failback": 1, "id": "service:vm:30001", "max_relocate": 3, "max_restart": 2,
     "node": "pve-dmz-node02-th-A", "request_state": "started", "sid": "vm:30001",
     "state": "started", "status": "vm:30001 (pve-dmz-node02-th-A, started)", "type": "service"},
]

# --- captured from Janus: GET /cluster/ha/status/manager_status (HTTP 200) ----------------
JANUS_HA_MANAGER_STATUS = {
    "lrm_status": {
        "pve-dmz-node01-th-A": {"mode": "active", "results": {}, "state": "wait_for_agent_lock",
                                "timestamp": 1789062565},
        "pve-dmz-node02-th-A": {"mode": "active", "results": {}, "state": "active",
                                "timestamp": 1789062567},
        "pve-dmz-node11-th-I": {"mode": "active", "results": {}, "state": "wait_for_agent_lock",
                                "timestamp": 1789062565},
        "pve-dmz-node12-th-I": {"mode": "active", "results": {}, "state": "wait_for_agent_lock",
                                "timestamp": 1789062565},
    ...
    "manager_status": {
        "master_node": "pve-dmz-node11-th-I",
        "node_request": {
            "pve-dmz-node01-th-A": {}, "pve-dmz-node02-th-A": {},
            "pve-dmz-node11-th-I": {}, "pve-dmz-node12-th-I": {},
            N21: {"maintenance": 1}, N22: {"maintenance": 1},
        },
        "node_status": {
            "pve-dmz-node01-th-A": "online", "pve-dmz-node02-th-A": "online",
            "pve-dmz-node11-th-I": "online", "pve-dmz-node12-th-I": "online",
            N21: "maintenance", N22: "maintenance",
        },
        "timestamp": 1789062561,
    },
    "quorum": {"node": "pve-dmz-node01-th-A", "quorate": "1"},
}

ALL_NODES = ['pve-dmz-node01-th-A', 'pve-dmz-node02-th-A', 'pve-dmz-node11-th-I',
             'pve-dmz-node12-th-I', N21, N22]
...
def _mgr(**extra):
    ...
    m.id = 'janus'
    m.current_host = None
    m.config = types.SimpleNamespace(host='pve-dmz-node01-th-A.uvensys.systems',
...
def test_status_current_lrm_shape_is_detected():
    ...
    m._api_get = _api_router({
        ...
        '/cluster/ha/status/current': _Resp({'data': JANUS_HA_STATUS_CURRENT}),
    })
...
def test_manager_status_structured_shape_is_detected():
    ...
    m = _mgr()
    m._api_get = _api_router({
        '/cluster/ha/status/manager_status': _Resp({'data': JANUS_HA_MANAGER_STATUS}),
    })
...
def test_maintenance_requested_but_not_yet_applied_still_counts():
    ...
    from pegaprox.core.manager import PegaProxManager
    import copy
    payload = copy.deepcopy(JANUS_HA_MANAGER_STATUS)
...
def test_empty_poll_is_logged_once_and_only_on_change(caplog):
    ...
    from pegaprox.core.manager import PegaProxManager
    empty = dict(JANUS_HA_MANAGER_STATUS)
    ...
    m._api_get = _api_router({'/cluster/ha/status/manager_status':
                              _Resp({'data': JANUS_HA_MANAGER_STATUS})})
    with caplog.at_level(logging.INFO, logger='test_maint_restart'):
        PegaProxManager._get_native_ha_maintenance_nodes(m)
        PegaProxManager._get_native_ha_maintenance_nodes(m)
...
def test_native_ha_maintenance_is_persisted(db, monkeypatch):
    ...
    rows = db.get_node_maintenance('janus')
...
def test_maintenance_survives_restart_and_node_is_not_a_target(db, monkeypatch):
    ...
    # nodes_in_maintenance) — the drained nodes carry the LOWEST scores, as on Janus.
    scores = {'pve-dmz-node01-th-A': 30.17, 'pve-dmz-node02-th-A': 54.94,
              'pve-dmz-node11-th-I': 30.53, 'pve-dmz-node12-th-I': 20.23,
              N21: 5.53, N22: 7.14}
    ...
    assert target == 'pve-dmz-node12-th-I'   # the emptiest node that is actually available
...
def test_restored_entry_is_not_dropped_by_the_next_poll(db, monkeypatch):
    ...
    exits. On Janus this matters because PVE can clear the LRM flag while guests are still on
    the node.
    """
    from pegaprox.core.manager import PegaProxManager
    import pegaprox.core.manager as mgrmod
...
def test_exit_clears_the_persisted_row(db, monkeypatch):
    ...
    assert db.get_node_maintenance('janus') == []
